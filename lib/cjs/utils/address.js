...
const getInjectiveAddress = (ethAddress) => {
    ...
    return bech32_2_1.bech32.encode(__1.AddressPrefix, bech32_2_1.bech32.toWords(addressBuffer));
};
...
const getEthereumAddress = (injectiveAddress) => {
    ...
    return `0x${Buffer.from(bech32_2_1.bech32.fromWords(bech32_2_1.bech32.decode(injectiveAddress).words)).toString("hex")}`;
};
...
const getDefaultSubaccountId = (injectiveAddress) => {
    return `0x${Buffer.from(bech32_2_1.bech32.fromWords(bech32_2_1.bech32.decode(injectiveAddress).words)).toString("hex")}${"0".repeat(24)}`;
};
...
const getSubaccountId = (injectiveAddress, nonce = 0) => {
    return `0x${Buffer.from(bech32_2_1.bech32.fromWords(bech32_2_1.bech32.decode(injectiveAddress).words)).toString("hex")}${"0".repeat(23)}${nonce}`;
};
...
const getAddressFromInjectiveAddress = (address) => {
    ...
    return `0x${Buffer.from(bech32_2_1.bech32.fromWords(bech32_2_1.bech32.decode(address).words)).toString("hex")}`;
};
