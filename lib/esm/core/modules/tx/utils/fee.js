...
const getStdFeeForToken = (token = { denom: __1.AddressPrefix, decimals: 18 }, gasPrice, gasLimit) => {
    const gasPriceInBase = gasPrice || new utils_1.BigNumberInWei(exports.DEFAULT_GAS_PRICE).toBase();
    const gasPriceScaled = new utils_1.BigNumberInBase(gasPriceInBase)
        .toWei(token.decimals)
        .toFixed(0);
    const gasNormalized = new bignumber_js_1.default(gasLimit || exports.DEFAULT_GAS_LIMIT).toFixed(0);
    return {
        amount: [
            {
                denom: token.denom,
...
const getStdFeeFromObject = (args) => {
    if (!args) {
        return utils_2.DEFAULT_STD_FEE;
    }
    const { gas = exports.DEFAULT_GAS_LIMIT.toString(), gasPrice = exports.DEFAULT_GAS_PRICE, payer, granter, feePayer, } = args;
...
const getDefaultStdFee = () => utils_2.DEFAULT_STD_FEE;
...
const getStdFeeFromString = (gasPrice) => {
    ...
    const gas = new utils_1.BigNumberInBase(amount)
        .toWei()
        .dividedBy(exports.DEFAULT_GAS_PRICE)
        .toFixed(0);
    return (0, exports.getStdFeeFromObject)({ gas, gasPrice: exports.DEFAULT_GAS_PRICE });
};
