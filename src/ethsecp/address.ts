...
export class Address {
   ...
   */
  static fromBech32(bech: string, prefix: string = 'dym'): Address {
    try {
   ...
   * @throws {Error} if bech is not a valid bech32-encoded address
   */
  static fromHex(hex: string, prefix: string = 'dym'): Address {
   ...
   * @returns {string}
   */
  toBech32(prefix: string = 'dym'): string {
  ...
  toValidatorAddress(): string {
    return this.toBech32('injvaloper');
  }
  ...
  toConsensusAddress(): string {
    return this.toBech32('injvalcons');
  }
